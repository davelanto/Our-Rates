...
<div class="container">
    <div class="text-center mx-auto mt-5 mb-5">
        ...
        <?php  $this->session = \Config\Services::session(); ?>
            ...
            <h5 class="text-uppercase">Sea Freight Rate</h5>
    ...
    <div class="row">
        ...
        <?php if($this->session->user_type == 'Editor'): ?>
            ...
            <div class="text-start col mb-2">
                <button class="btn btn-sm btn-dark" data-bs-toggle="modal" data-bs-target="#asp_sea_freight_modal">New Rate <i class="fas fa-plus fa-fw"></i></button>
            </div>

        <?php endif; ?>

        <div class="text-end col mb-2">
            <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#asp_sea_freight_notes">Editor's note<i class="far fa-clipboard fa-fw"></i></button>
        </div>

    </div>

    <h6 class="text-secondary">Domestic Sea freight</h6>
    ...
    <table class="table table-striped table-responsive" style="width:100%; overflow-x:auto;">
            ...
            <tbody>
                ...
                <?php if($aspseafreight): ?>

                    <?php foreach($aspseafreight->getResult() as $rate ): ?>

                        <tr>
                            <?php if($this->session->user_type == 'Editor'): ?>
                                
                                <td><button type="button" class="btn btn-primary btn-sm edit" id="edit" >Edit</button></td>
                                <td><a href="<?=base_url('aspseafreight/delete/'. $rate->id); ?>" class="btn btn-sm btn-danger">Delete</a></td>
    ...
    <?php if($this->session->user_type == 'Editor'): ?>
        ...
        <form method="post" action="<?=base_url('aspseafreight/add');?>" class="needs-validation" novalidate >
            
            <div class="modal fade" id="asp_sea_freight_modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="asp_air_freight_modallabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h6 class="modal-title fw-bold poppins text-secondary" id="asp_air_freight_modallabel">Sea Freight Rate</h6>
    ...
    <?php endif; ?>
        ...
        <form action="<?=base_url('aspseafreight/updatenotes');?>" method="post">

             <div class="modal fade" id="asp_sea_freight_notes" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="asp_air_freight_noteslabel" aria-hidden="true">
...
<script src="<?=base_url('js/aspseafreight.js');?>"></script>