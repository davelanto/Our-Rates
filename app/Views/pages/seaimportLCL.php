...
<?php  $this->session = \Config\Services::session(); ?>
    ...
    <div class="text-center mx-auto mt-5 mb-5">
        ...
        <h5 class="text-uppercase text-danger">LOOSE CONTAINER LOAD DESTINATION CHARGES <span class="fst-italic fw-light">(LCL)</span> </h5>
    ...
    <div class="row">
        ...
        <?php if($this->session->user_type == 'Editor'): ?>
            ...
            <div class="text-start col mb-2">
                <button class="btn btn-sm btn-dark" data-bs-toggle="modal" data-bs-target="#seaimportLCLModal">New Rate <i class="fas fa-plus fa-fw"></i></button>
            </div>

        <?php endif; ?>

        <div class="text-end col mb-2">
            <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#seaimportLCLNotesModal">Editor's note<i class="far fa-clipboard fa-fw"></i></button>
        </div>
    ...
    <table class="table table-striped  table-responsive" style="width:100%; overflow-x:auto;" >
        ...
        <tbody >
            ...
            if ($seaimportLCL) {
                ...
                foreach($seaimportLCL->getResult() as $rate)
                {
            ?>
                <tr>
                <?php if($this->session->user_type == 'Editor'): ?>
                        <td class="col-action"><button type="button" class="btn btn-primary btn-sm edit" id="edit" >Edit</button></td>
                        <td class="col-action"><a href="<?=base_url('seaimportLCL/delete/'. $rate->id); ?>" class="btn btn-sm btn-danger">Delete</a></td>
                <?php endif; ?>
    ...
    <form method="POST" action="<?=base_url('seaimportLCL/add');?>" class="needs-validation" novalidate>
    <div class="modal fade" id="seaimportLCLModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="seaimportLCLModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                    <div class="modal-content">
                            <div class="modal-header">
                                <h6 class="modal-title fw-bold poppins text-secondary" id="seaimportLCLModalLabel">Sea import Rate <br> <span class="text-danger fw-light"> LCL Destination Charges </span> </h6>
    ...
    <form action="<?=base_url('seaimportLCL/updatenotes');?>" method="post">
            <div class="modal fade" id="seaimportLCLNotesModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="seaimportLCLNotesModalLabel" aria-hidden="true">
                                <div class="modal-dialog  modal-lg modal-dialog-centered modal-dialog-scrollable">
                                        <div class="modal-content">
                                                <div class="modal-header">
                                                    <h6 class="modal-title fw-bold poppins text-secondary" id="seaimportLCLNotesModalLabel">Editor's Note <br> <span class="text-danger text-lgi"></span> </h6>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
...
<script src="<?=base_url('js/seaimportlcl.js');?>"></script>